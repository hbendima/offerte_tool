...
export default function QuotationTool({ onOfferteSaved }) {
  ...
  useEffect(() => {
    ...
    products.forEach(p => {
      ...
      if (inStock > 0) {
        cdc += -5.33;
        cdcDetails.push(`${p.SKU}: ${inStock} uit voorraad, CDC -5.33`);
      }
      if (backOrder > 0) {
        cdc += -5.33;
        cdcDetails.push(`${p.SKU}: ${backOrder} backorder, CDC -5.33`);
      }
    });
                    ...
                    >
                      ...
                      {totals.cdcDetails && totals.cdcDetails.length > 0 && (
                        <ul style={{ marginTop: 6 }}>
                          {totals.cdcDetails.map((d, i) => <li key={i}>{d}</li>)}
                        </ul>
                      )}
                    </div>
                  )}
